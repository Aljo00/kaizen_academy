...
const STORY_VIDEO =
  "https://customer-assets.emergentagent.com/job_kaizen-premium/artifacts/2iedaedl_WhatsApp%20Video%202026-06-17%20at%209.39.20%20PM.mp4";
  ...
  return (
    <main data-testid="about-page" className="pt-32">
      ...
      <section className="relative max-w-7xl mx-auto px-5 md:px-12 pb-12 md:pb-24">
        ...
        <div className="text-[10px] md:text-[11px] tracking-[0.3em] uppercase text-slate-500 font-semibold">
        ...
        <h1 className="font-display mt-4 md:mt-5 text-4xl sm:text-5xl lg:text-7xl tracking-tight font-light text-slate-900 max-w-4xl text-balance leading-[1.05]">
          We don't run a tuition centre. We build{" "}
        ...
        <p className="mt-5 md:mt-6 max-w-2xl text-base md:text-lg text-slate-600 leading-relaxed">
          Born in Thrithala, Kerala, Kaizen Academy is a premium coaching
          institution committed to nurturing students through personalised
          mentorship, modern teaching methodology and career-oriented education.
        </p>
      </section>

      {/* OUR STORY VIDEO */}
      <section
        className="relative py-16 md:py-28 bg-[#0B1220] overflow-hidden"
        data-testid="about-story-video"
      >
        ...
        <div className="relative max-w-6xl mx-auto px-5 md:px-12">
          <div className="text-center max-w-2xl mx-auto text-white">
            ...
            <h2 className="font-display mt-4 text-3xl sm:text-4xl md:text-5xl font-light tracking-tight">
              The story of{" "}
              <em className="ka-gradient-text not-italic">Kaizen Academy</em>.
            </h2>
            <p className="mt-4 text-sm md:text-base text-slate-300 leading-relaxed">
              A glimpse into our journey — the classrooms, the students, the
              spirit that makes Kaizen what it is today.
            </p>
          </div>
          <div
            className="mt-10 md:mt-14 relative rounded-2xl md:rounded-3xl overflow-hidden shadow-[0_30px_80px_-30px_rgba(0,0,0,0.6)] ring-1 ring-white/10 bg-black aspect-video group cursor-pointer"
            onClick={() => setOpen(true)}
            data-testid="about-story-thumb"
          >
            <video
              src={STORY_VIDEO}
              muted
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white text-xs md:text-sm font-medium tracking-wide">
              Tap to watch full story
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-12 grid lg:grid-cols-12 gap-8 md:gap-12 items-center">
          ...
          <div className="lg:col-span-7">
            ...
            <p className="mt-5 md:mt-6 text-sm md:text-base text-slate-600 leading-relaxed">
              Kaizen — the Japanese philosophy of continuous improvement — is
              the heartbeat of our academy. Every student, every batch, every
              day, we get a little better at what we do.
            </p>
            <div className="mt-7 md:mt-8 flex items-center gap-4">
              <div className="h-px flex-1 bg-slate-200" />
              <div>
                <div className="font-display text-lg md:text-xl italic text-slate-900">
                  Suhail
                </div>
                <div className="text-[10px] md:text-xs tracking-[0.2em] uppercase text-slate-500">
          ...
          <div className="mt-10 md:mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
            {WHY_FEATURES.map((f, i) => {
              ...
              return (
                <div
                  key={f.title}
                  className="p-5 md:p-7 rounded-2xl bg-white border border-slate-100 hover:-translate-y-1 hover:shadow-lg transition-all"
                  data-testid={`value-${i}`}
                >
                  <div className="inline-flex h-10 w-10 md:h-11 md:w-11 rounded-xl bg-slate-900 text-white items-center justify-center">
                    <Icon className="h-5 w-5" />
                  </div>
                  <div className="mt-4 md:mt-5 font-display text-lg md:text-xl text-slate-900">
                    {f.title}
                  </div>
        ...
        <div className="max-w-5xl mx-auto px-5 md:px-12 text-center">
          ...
          <div className="mt-10 md:mt-14 grid grid-cols-2 md:grid-cols-4 gap-px bg-slate-200 rounded-2xl md:rounded-3xl overflow-hidden">
            {STATS.map((s, i) => (
              <div
                key={i}
                className="bg-white p-5 md:p-8"
                data-testid={`about-stat-${i}`}
              >
                <div className="font-display text-3xl md:text-5xl font-light text-slate-900">
                  <AnimatedCounter value={s.value} suffix={s.suffix} />
                </div>
                <div className="mt-2 text-[10px] md:text-xs tracking-[0.2em] uppercase text-slate-500">
                  {s.label}
                </div>
              </div>
            ))}
      ...
      <section className="py-16 md:py-32 bg-[#FAFAFA]">
        <div className="max-w-7xl mx-auto px-5 md:px-12">
          ...
          <div className="mt-10 md:mt-12 columns-2 md:columns-2 lg:columns-3 gap-3 md:gap-5">
            {[...ASSETS.campus, ...ASSETS.activities.map((a) => a.src)]
              ...
              .map((src, i) => (
                <div
                  key={src}
                  className="mb-3 md:mb-5 break-inside-avoid group rounded-xl md:rounded-2xl overflow-hidden bg-slate-100 border border-slate-100"
                  data-testid={`about-campus-${i}`}
                >
                  <img
                    src={src}
                    alt="campus"
          ...
          <div className="mt-8 md:mt-12 flex justify-center">
            <Link
              to="/gallery"
              className="group inline-flex items-center gap-2 px-6 md:px-7 py-3 md:py-3.5 rounded-full bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 hover:-translate-y-0.5 transition-all shadow-[0_10px_30px_rgba(15,23,42,0.18)]"
              data-testid="about-view-all-btn"
            >
              View All Photos{" "}
